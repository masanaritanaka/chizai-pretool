...
【タスク: 特許文書をやさしく読み解く（他社特許の読解用）】
入力された特許文書テキスト（請求の範囲・要約・明細書の一部）を、知財の専門知識がない事業担当者・エンジニアが「自分の活動と抵触するか」を判断する出発点として読めるように解説してください。

【前提: 読み手の第一関心は以下の2点です】
1. この特許の権利範囲はどこまでか（何をしたら踏むのか）
2. この特許は今どういう状態か（生きているのか、どこまで有効なのか）

【出力の注意事項】
- 「クレーム」「請求項」などの専門語は必ず平易語を並記する（例: 「保護範囲(請求の範囲)」）
- 「均等論」「進歩性」「拒絶理由」などの審査用語は使わない
- 各項目は結論から書き始める。前置き・導入文で始めない
- 【重要】AIは現在の権利状態（登録済み・存続中・失効・拒絶）を断定してはならない。
  公報テキストからは出願・公開の事実しか読めない。
  statusセクションでは必ず「経過情報で確認してください」の導線を示すこと。

- summary: この特許が「何を」「どうやって」「何のために」実現するものか。専門用語なしの2〜3文
- usageScenes: 実生活・実務での想像しやすい適用シーン。具体的な製品・場面を例示
- claimScope: 「この特許が押さえている範囲(保護範囲)」を平易に説明する。
  ・どの構成要素が必須か（それがないと権利範囲に入らない部分）を明示する
  ・広い権利か狭い権利かの印象も含める
  ・「〇〇という方法を、〇〇という手段で行う場合に」のように具体的に書く
- termMap: 文書中の重要語を「原文語 → 平易な言い換え」として5〜8語
- whatItMeansForReaders: 読み手が自社活動を照らし合わせる観点。
  ・「この特許に近いことをしていないか」のチェックポイント（1〜2文）
  ・「回避の余地がありそうな点」（1〜2文）
  ※判断は促さず観点の提示に留める。「〜かどうか確認を」「〜の余地がある可能性」の表現を使う
- status: 出願日・公開日・優先日など、文書から読み取れる情報を平易に説明する。
  「古い特許か新しいか」「分割出願の可能性があるか」などを言及してよい。
  必ず末尾に「現在登録されているか・権利が存続しているかは、J-PlatPatの経過情報で確認してください」を含めること
`.trim();
...

{
  ...
  "claimScope": "string（保護範囲の平易な説明。必須構成要素の特定を含む）",
  "termMap": [{"term": "原文語（string）", "plain": "平易な言い換え（string）"}],
  "whatItMeansForReaders": "string（読み手が自社活動を照らす観点。判断は促さず観点提示のみ）",
  "status": "string（文書から読み取れる出願日・公開日等の情報。必ず経過情報確認の導線を含む）"
}
`.trim();
